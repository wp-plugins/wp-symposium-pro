...
function wps_update_routine() {
	...
	// Read and hide release notes?
	if (is_blog_admin() && current_user_can('manage_options') && isset($_GET['wps_hide_release_notes'])):
		delete_option('wp_symposium_pro_ver_show_release_notes');
	endif;

	if (is_blog_admin() && current_user_can('manage_options') && get_option('wp_symposium_pro_ver_show_release_notes') && ((!isset($_GET['page'])) || (isset($_GET['page']) && $_GET['page'] != 'wps_pro_release_notes'))):
	...
	if (is_blog_admin() && current_user_can('manage_options') && get_option('wp_symposium_pro_ver') != $new_version):
		...
		// Flag to show release notes on next page reload/refresh
		update_option('wp_symposium_pro_ver_show_release_notes', true);		
	endif;	
}
