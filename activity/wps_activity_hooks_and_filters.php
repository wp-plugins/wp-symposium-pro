			...
			if (post_type_exists('wps_alerts') && count($recipients) > 0):
				...
				foreach ($recipients as $target_id):
					...
					if ( (int)$target_id != (int)$current_user->ID && !in_array($target_id, $sent) ):
						...
						if (get_user_meta($target_id, 'wps_activity_subscribe', true) != 'off'):
							...
							$content .= '<p>'.sprintf(__('You have a new post on your activity from %s.', WPS2_TEXT_DOMAIN), $author->display_name).'</p>';
							$content .= '<p><em>'.$the_post['wps_activity_post'].'</em></p>';
							
							if ( get_option( 'permalink_structure' ) ):	
								$u = get_user_by('id', $the_post['wps_activity_post_author']);
								$parameters = sprintf('%s?view=%d', $u->user_login, $new_id);
								$permalink = get_permalink(get_option('wpspro_profile_page'));
								$url = $permalink.$parameters;
							else:
								$parameters = sprintf('user_id=%d&view=%d', $the_post['wps_activity_post_author'], $new_id);
								$permalink = get_permalink(get_option('wpspro_profile_page'));
								$url = $permalink.'&'.$parameters;
							endif;
					...
					if ($value):
						...
						if ( (int)$value != (int)$current_user->ID && !in_array($value, $sent) ):
							...
							if (get_user_meta($value, 'wps_activity_subscribe', true) != 'off'):
								...
								$content .= '<p>'.sprintf(__('A new comment from %s.', WPS2_TEXT_DOMAIN), $author->display_name).'</p>';
