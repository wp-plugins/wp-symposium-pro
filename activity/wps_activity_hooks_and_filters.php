...
function wps_activity_post_add_alerts($the_post, $the_files, $new_id) {
	...
	if (post_type_exists('wps_alerts')):
		...
		if ($get_recipient && $get_the_post->wps_target != $the_post['wps_activity_post_author']) {
			...
			// Up until 15.5 a post on activity generated alerts to all friends, this could
			// cause spam, and negates the value of tagging. Therefore, from 15.5, this is disabled,
			// and alerts can be sent to friends via the whoto extension or tagging. An admin option
			// maybe added at a later date to make this an option.
			/*
			global $current_user;	
