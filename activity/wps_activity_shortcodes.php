	...
	if (is_user_logged_in()) {
		...
		if ($current_user->ID == $user_id || $friends['status'] == 'publish'):
			...
			if (!$post_id):
				...
				if ($type == ''): // Activity only
					...
					if ($include_self):
						...
						$results = $wpdb->get_results($wpdb->prepare($sql, 'wps_activity', $user_id, $current_user->ID, $user_id, $current_user->ID, $user_id, $user_id, $current_user->ID, $current_user->ID, $count));
							...
							if (!$actvity_type):
								...
								if ($add):
									if ($r->post_date > $r->comment_date):
										array_push($activity, array('ID' => $r->ID, 'date' => $r->post_date));
									else:
										array_push($activity, array('ID' => $r->ID, 'date' => $r->comment_date));
									endif;
								endif;
							endif;
						endforeach;
				...
				array_push($activity, array('ID' => $post_id, 'date' => $single->post_date));
			...
			if ($activity):
				...
				// Sort array by dates, newest first
				usort($activity, "wps_cmp_dates");			

				$html .= '<div id="wps_activity_items" style="display:none">';
					...
					foreach ($activity as $i):
						...
						if (!in_array($i['ID'], $shown)):
							...
							$item_html .= '<div class="wps_activity_item" id="wps_activity_'.$item->ID.'" style="position:relative;padding-left: '.($avatar_size+10).'px">';
								...
								$item_html .= '<div id="wps_activity_'.$item->ID.'_content" class="wps_activity_content">';
									...
									if ($item->post_author == $current_user->ID || current_user_can('manage_options')):
										$item_html .= '<div class="wps_activity_settings">';
											$item_html .= '<img src="'.plugins_url('images/wrench'.get_option('wpspro_icon_colors').'.svg', __FILE__).'" />';
										$item_html .= '</div>';
										$item_html .= '<div class="wps_activity_settings_options">';
											$item_html .= '<a class="wps_activity_settings_delete" rel="'.$item->ID.'" href="javascript:void(0);">'.__('Delete this post', WPS2_TEXT_DOMAIN).'</a>';
										$item_html .= '</div>';
									endif;

									// Avatar
									$item_html .= '<div class="wps_activity_item_avatar" style="margin-left: -'.($avatar_size+10).'px">';
									...
									$item_html .= '<div class="wps_activity_item_meta">';
										...
										$item_html .= '<br />';
										$item_html .= '<div class="wps_ago">'.sprintf('%s ago', human_time_diff(strtotime($item->post_modified), current_time('timestamp')), WPS2_TEXT_DOMAIN).'</div>';
										...
										foreach($comments as $comment) :
											$item_html .= '<div id="wps_comment_'.$comment->comment_ID.'" class="wps_activity_comment" style="position:relative;padding-left: '.($comment_avatar_size+10).'px">';
												...
												if ($comment->user_id == $current_user->ID || current_user_can('manage_options')):
													$item_html .= '<div class="wps_comment_settings">';
														$item_html .= '<img src="'.plugins_url('images/wrench'.get_option('wpspro_icon_colors').'.svg', __FILE__).'" />';
													$item_html .= '</div>';
												...
												// Avatar
												$item_html .= '<div class="wps_activity_post_comment_avatar" style="margin-left: -'.($comment_avatar_size+10).'px">';
												...
												$item_html .= '<br />';
												$item_html .= '<div class="wps_ago">'.sprintf('%s ago', human_time_diff(strtotime($comment->comment_date), current_time('timestamp')), WPS2_TEXT_DOMAIN).'</div>';
