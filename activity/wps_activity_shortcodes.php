...
function wps_activity_page($atts){
	...
	$html .= wps_display_name(array('before'=>'<div id="wps_display_name" style="font-size:2.5em; margin-bottom:20px;">', 'after'=>'</div>'));
	...
	$html .= wps_friends_pending(array('count' => 1, 'before'=>'<strong>'.__('Friend Requests', WPS2_TEXT_DOMAIN).'</strong><br />'));
	...
	if ($user_can_see_activity):
		...
		if (!$post_id):
					...
					foreach ($results as $r):
                        ...
                        if (!$actvity_type):
                            ...
                            if ($user_id == $current_user->ID):		// ------------ On user's own page
                                ...
                                if ($r->post_author == $current_user->ID && in_array((string)$user_id, $target_users)) { $add = true; };
					...
					if ($friends):
						foreach ($friends as $friend):
							...
							foreach ($results as $r):
								...
								if ($add):
									$user_can_see_activity = apply_filters( 'wps_check_activity_security_filter', $add, $r->post_author, $current_user->ID );
			...
			if ($single):
				...
				if (is_array($target_ids)):
					// Show if this user is in the list of target user IDs
					if (in_array((string)$current_user->ID, $target_ids)) { $add = true; };
				else:
					// Show if this user is the target, or the user is posting to all friends
					if ($current_user->ID == $target_ids || $single->post_author == $target_ids) $add = true;
				endif;
				// Check that author's permissions for their activity
				if ($add):
					$user_can_see_activity = apply_filters( 'wps_check_activity_security_filter', $add, $single->post_author, $current_user->ID );
					if (!$user_can_see_activity) $add = false;
				endif;

				// Current user is the author, always show
				if ($single->post_author == $current_user->ID) { $add = true; };

				if ($add) array_push($activity, array('ID' => $post_id, 'date' => $single->post_date, 'is_sticky' => 0));														

				// Any more activity?
				$activity = apply_filters( 'wps_activity_single_item_filter', $activity, $atts, $user_id, $current_user->ID );
                            ...
                            $item_html .= '<div class="wps_activity_item'.$is_sticky_css.'" id="wps_activity_'.$item->ID.'" style="position:relative;padding-left: '.($avatar_size+10).'px">';
                                ...
                                $item_html .= '<div id="wps_activity_'.$item->ID.'_content" class="wps_activity_content">';
                                    ...
                                    $item_html .= '<div class="wps_activity_item_meta">';
                                        ...
                                        // Any more meta?
                                        // Passes $item_html, shortcodes options ($atts), current post ID ($item->ID), user page ($user_id), current users ID ($current_user->ID)
                                        $item_html = apply_filters( 'wps_activity_item_meta_filter', $item_html, $atts, $item->ID, $user_id, $current_user->ID );
                                    ...
                                    if ($i=strpos($p, '[items]')):
                                        $attachments = substr($p, $i+7, strlen($p)-($i+9));
                                        $p = substr($p, 0, $i);
                                        ...
                                        foreach ($attachments as $attachment):
                                            $attachment_html .= '<div class="wps_activity_item_attachment wps_activity_item_attachment_item">'.wp_get_attachment_image($attachment, 'thumbnail');            
                                                ...
                                                $attachment_html .= '<div class="wps_activity_item_attachment_full">'.$image_src[0].'</div>';
                                            $attachment_html .= '</div>'; 
                                        endforeach;
                                        $attachment_html .= '</div>';
                                    ...
                                    $item_html .= '<div class="wps_activity_item_post" id="activity_item_'.$i['ID'].'">'.$p.'</div>';
                                    ...
                                    if ($comments) {
                                        ...
                                        foreach($comments as $comment) :
                                            ...
                                            $item_html .= '<div id="wps_comment_'.$comment->comment_ID.'" class="wps_activity_comment wps_activity_item_'.$item->ID.'" style="'.$hidden_style.'position:relative;padding-left: '.($comment_avatar_size+10).'px">';
                                                ...
                                                // Any other meta
                                                // Passes $item_html, shortcodes options ($atts), current post ID ($item->ID), current comment ID ($comment->comment_ID), user page ($user_id), current users ID ($current_user->ID)
                                                $item_html = apply_filters( 'wps_activity_comment_meta_filter', $item_html, $atts, $item->ID, $comment->comment_ID, $user_id, $current_user->ID );                                    
		                                        ...
		                                        $item_html = apply_filters( 'wps_activity_post_comment_filter', $item_html, $atts, $item->ID, $comment->comment_ID, $user_id, $current_user->ID );                                                
                                ...
                                // Add new comment	
                                if (is_user_logged_in() && $allow_replies):						
                                    ...
                                    $add_form .= '</div>';
                                    $add_form = apply_filters( 'wps_activity_new_comment_filter', $add_form, $atts, $item->ID, $user_id, $current_user->ID );
                                    $item_html .= $add_form;
                                endif;
