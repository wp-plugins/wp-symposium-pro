...
if ($debug) $debug_html .= 'activity from self<br />'.$wpdb->prepare($sql, 'wps_activity', $user_id, $this_user, $user_id, $this_user, $user_id, $user_id, $this_user, $this_user, $get_max).'<br /><br />';
                    ...
                    if ($include_friends):
                        ...
                        $sql = "SELECT user_id FROM ".$wpdb->prefix."usermeta WHERE meta_key = 'wpspro_last_active' AND STR_TO_DATE(meta_value,'%%Y-%%m-%%d %%H:%%i:%%s') >= date(now()-interval %d day)";
...
if ($debug) $debug_html .= 'activity from all friends<br />'.$wpdb->prepare($sql, 'wps_activity', $friend['ID'], $this_user, $count).'<br /><br />';
