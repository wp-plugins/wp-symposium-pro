jQuery(document).ready(function() {
	...
	jQuery('#wps_activity_settings_options').show();
	...
	jQuery(".wps_activity_post_comment_button").click(function (event) {
		...
		if (comment.length) {
			...
			comment_div += '</div>';
			jQuery('#wps_activity_'+id+'_content').append(comment_div);
	...
	jQuery(".activity_item_more").click(function (event) {
		var id = jQuery(this).attr('rel');
		jQuery('#activity_item_'+id).slideUp('fast');
		jQuery('#activity_item_full_'+id).slideDown('fast');
	});
