...
function user_avatar_add_photo_step1($uid)
{
	...
	<form enctype="multipart/form-data" id="uploadForm" method="POST" action="<?php echo admin_url('admin-ajax.php'); ?>?action=user_avatar_add_photo&step=2&uid=<?php echo $uid; ?>" >
		...
		<p class="submit"><input type="submit" class="btn btn-primary" value="<?php esc_attr_e('Upload'); ?>" /></p>
	</form>
	</div>
...
function user_avatar_add_photo_step2($uid)
{
		...
		<input type="submit" class="btn btn-primary" style="margin-left:0;" id="user-avatar-crop-button" value="<?php esc_attr_e('Crop Image', WPS2_TEXT_DOMAIN); ?>" />
		</div>
		</form>
...
function user_avatar_form($profile)
{
	...
	<a id="user-avatar-link" class="button-secondary thickbox" href="<?php echo admin_url('admin-ajax.php'); ?>?action=user_avatar_add_photo&step=1&uid=<?php echo $profile->ID; ?>&TB_iframe=true&width=720&height=450" title="<?php _e('Upload and Crop an Image to be Displayed', WPS2_TEXT_DOMAIN); ?>" ><?php _e('Update Picture', WPS2_TEXT_DOMAIN); ?></a> 
