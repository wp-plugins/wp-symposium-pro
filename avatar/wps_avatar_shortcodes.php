...
function wps_avatar_init() {
	...
	wp_enqueue_style('user-avatar', plugins_url('css/user-avatar.css', __FILE__), 'css');
...
function wps_avatar_change($atts) {
	...
	if (is_user_logged_in()):
		...
		if ($current_user->ID == $user_id || current_user_can('edit_user', $current_user->ID) || is_super_admin($current_user->ID) ):
			...
			if (!isset($_POST['wps_avatar_change_step'])):
				...
				$html .= '<form enctype="multipart/form-data" id="avatarUploadForm" method="POST" action="#" >';
					...
					$html .= '<input title="Choose an image" type="file" id="upload" name="uploadedfile" /><br /><br />';
					wp_nonce_field('user-avatar');
					$html .= '<input type="submit" class="" value="'.$label.'" />';
				$html .= '</form>';

			elseif ($_POST['wps_avatar_change_step'] == '2'):
				...
				if (!(($_FILES["uploadedfile"]["type"] == "image/gif") || ($_FILES["uploadedfile"]["type"] == "image/jpeg") || ($_FILES["uploadedfile"]["type"] == "image/png") || ($_FILES["uploadedfile"]["type"] == "image/pjpeg") || ($_FILES["uploadedfile"]["type"] == "image/x-png"))):
					...
					$html .= "<div class='error'><p>".__("Please upload an image file (.jpeg, .gif, .png).", WPS2_TEXT_DOMAIN)."</p></div>";
					...
					wp_nonce_field('user-avatar');
					$html .= '<input type="submit" class="" style="margin-left:20px;" id="user-avatar-crop-button" value="'.__('Crop', WPS2_TEXT_DOMAIN).'" />';
					$html .= '</form>';
				...
				if( !file_exists($original_file) ):
					...
					// update the files 
					$cropped_full = wp_crop_image( $original_file, $_POST['x1'], $_POST['y1'], $_POST['width'], $_POST['height'], 150, 150, false, $cropped_full );
					$cropped_thumb = wp_crop_image( $original_file, $_POST['x1'], $_POST['y1'], $_POST['width'], $_POST['height'], 150, 150, false, $cropped_thumb );
