...
if ($show_header):
	$html .= '<div class="wps_forum_posts_header">';
		$html .= '<div class="wps_forum_title_header">'.__('Post', WPS2_TEXT_DOMAIN).'</div>';
		$html .= '<div class="wps_forum_count_header">'.__('Replies', WPS2_TEXT_DOMAIN).'</div>';
		$html .= '<div class="wps_forum_last_poster_header">'.__('Last activity', WPS2_TEXT_DOMAIN).'</div>';
	$html .= '</div>';
endif;
		...
		foreach ($forum_posts as $forum_post):
			...
			if ($forum_post['post_status'] == 'publish' || current_user_can('edit_posts') || $forum_post['post_author'] = $current_user->ID):
				...
				if ($comments):
					...
					$created = sprintf(__('%s ago', WPS2_TEXT_DOMAIN), human_time_diff(strtotime($comments[0]->comment_date_gmt), current_time('timestamp', 1)), WPS2_TEXT_DOMAIN);
					...
					$author = wps_display_name(array('user_id'=>$forum_post['post_author'], 'link'=>1));
					$created = sprintf(__('%s ago', WPS2_TEXT_DOMAIN), human_time_diff(strtotime($forum_post['post_date_gmt']), current_time('timestamp', 1)), WPS2_TEXT_DOMAIN);
					$comment_count = 0;
				endif;
