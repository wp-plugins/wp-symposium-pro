...
function wps_forum_comment($atts) {
	...
	if ((!isset($_GET['forum_action']) || ($_GET['forum_action'] != 'edit' && $_GET['forum_action'] != 'delete')) && get_query_var('topic') ): // showing a single post
		...
		extract( shortcode_atts( array(
            ...
            'allow_private' => 0,
            'private_reply_check_msg' => __('Only share reply with %s', WPS2_TEXT_DOMAIN),
			'slug' => '',
			'before' => '',
			'after' => '',
                                    ...
                                    $form_html .= '<div id="wps_forum_comment_div">';
                                        ...
                                        $form_html .= '<div id="wps_forum_comment_form"';
                                            ...
                                            if ($allow_private):
                                                ...
                                                if ($originator) $form_html .= '<input type="checkbox" name="wps_private_post" id="wps_private_post" style="float:left; margin-right:10px;" /><label for="wps_private_post">'.sprintf($private_reply_check_msg, $originator->display_name).'</label>';
                                            endif;
...
function wps_forum_post($atts) {
	...
	if ( $show_forum ): // not showing a single post or just deleted a topic
		...
		extract( shortcode_atts( array(
			...
			'private_msg' => '',
			'moderate' => false,
            'multiline' => false, // set to number of lines
			'show' => 0,
			'slug' => '',
			'before' => '',
			'after' => '',
...
function wps_forum($atts) {
	...
	extract( shortcode_atts( array(
		'slug' => '',
        'style' => 'table', // layout look and feel, table|classic
        // table... (default)
		'show_header' => true,
	    'show_closed' => 1,
        'show_count' => 1,
        ...
        // classic...
        'started' => __('Started by %s %s', WPS2_TEXT_DOMAIN),
        'replied' => __('Last replied to by %s %s', WPS2_TEXT_DOMAIN),
        'commented' => __('Last commented on by %s %s', WPS2_TEXT_DOMAIN),
        'size_posts' => 96,
        'size_replies' => 48,
        'post_preview' => 250,
        'reply_preview' => 120,
        'view_count_label' => __('VIEW', WPS2_TEXT_DOMAIN),
        'views_count_label' => __('VIEWS', WPS2_TEXT_DOMAIN),
        'reply_count_label' => __('REPLY', WPS2_TEXT_DOMAIN),
        'replies_count_label' => __('REPLIES', WPS2_TEXT_DOMAIN),
        // all layout options...
        'title_length' => 150,        
		'parent' => 0,
		...
		'timeout' => 120,
		'count' => -1,
        'size' => 96, // size of user avatar's on single post view
        ...
        'reply_icon' => 1,
		'base_date' => 'post_date_gmt',
		'comment_base_date' => 'comment_date_gmt',
		'before' => '',
		'after' => '',
	), $atts, 'wps_forum' ) );	
			...
			foreach ($forums as $forum):
                ...
                $forum_html .= '<div class="wps_forum_featured_content" style="position:relative; padding-left: '.($featured_image_width+$featured_image_padding).'px; ">';
                    ...
                    $forum_html .= '<div class="wps_forum_featured_image" style="margin-left: -'.($featured_image_width+$featured_image_padding).'px;float: left;width: '.$featured_image_width.'px">';
                        $forum_html .= '<img src="'.$featured_image.'" />';
                    $forum_html .= '</div>';
                    ...
                    $forum_html .= '<div style="width: 100%; " class="wps_forum_categories_item wps_forum_categories_item_'.$heading_indent_h.'">';
                        ...
                        if ($show_posts):
                            ...
                            $forum_html .= wps_forum(array('style' => 'table', 'slug' => $forum['slug'], 'base_date' => $base_date, 'status' => $show_closed, 'show_header' => $show_posts_header, 'count' => $show_posts, 'header_title'=>$header_title, 'header_count'=>$header_count, 'header_last_activity' => $header_last_activity, 'title_length' => $title_length, 'show_count' => $show_count, 'show_last_activity' => $show_last_activity, 'show_freshness' => $show_freshness));
                            $forum_html .= '</div>';
                        endif;
