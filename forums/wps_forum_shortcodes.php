...
function wps_forum_page($atts) {
	...
	extract( shortcode_atts( array(
		'slug' => '',
		'header_title' => __('Post', WPS2_TEXT_DOMAIN),
				    ...
				    foreach( $post_terms as $term ):
				    	...
				    	if (!$slug || $slug == $term->slug):
							...
							if (user_can_see_forum($current_user->ID, $term->term_id)):
								...
								if (!$wps_forum_author || $forum_post['post_author'] == $current_user->ID || current_user_can('manage_options')):
...
function wps_forums($atts) {
	...
	$html = wps_forum_categories_children($html, $parent, $show_children, $atts, 0);
...
function wps_forum_categories_children($html, $forum_id, $show_children, $atts, $indent) {
		...
		if ($forums):
			...
			foreach ($forums as $forum):
				...
				if ($show_posts):
					...
					$forum_html .= wps_forum(array('slug'=> $forum['slug'], 'show_header' => $show_posts_header, 'show_posts' => $show_posts, 'header_title'=>$header_title, 'header_count'=>$header_count, 'header_last_activity' => $header_last_activity));
				endif;
				...
				if ($show_children)
					$html = wps_forum_categories_children ($html, $forum['term_id'], $show_children, $atts, $indent+1);
