...
function wps_forum_page($atts) {
	...
	if ($slug == ''):
		...
		$html .= wps_forum(array('slug'=>$slug));
	...
	if ($include_posts):
		...
		if ($loop->have_posts()):
			...
			while ( $loop->have_posts() ) : $loop->the_post();
				...
				if ($status == 'all' || $status == $post->comment_status):
								...
								$forum_html .= '<div class="wps_forum_get_post">';
									...
									if ($show_date):
										$forum_html .= '<div class="wps_forum_get_date">';
											if ($date_format != 'ago'):
												$forum_html .= date($date_format, strtotime($forum_post['post_date_gmt']));
											else:
												$forum_html .= sprintf(__('%s ago', WPS2_TEXT_DOMAIN), human_time_diff(strtotime($forum_post['post_date_gmt']), current_time('timestamp', 1)), WPS2_TEXT_DOMAIN);
											endif;
										$forum_html .= '</div>';										
									endif;
...
function wps_forum_backto($atts) {
	...
	if ( !isset($_GET['forum_action']) && get_query_var('topic') ): // showing a single post
...
function wps_forum_comment($atts) {
	...
	if (is_user_logged_in() && !isset($_GET['forum_action']) && get_query_var('topic') ): // showing a single post
		...
		if ($slug == ''):
			...
			if (user_can_see_forum($current_user->ID, $term->term_id)):
				...
				if ( $my_posts ):
					...
					$html .= __('Unable to find forum post ID', WPS2_TEXT_DOMAIN);
	...
	if ($slug == ''):
		...
		if (user_can_see_forum($current_user->ID, $term->term_id)):
			...
			if ( get_query_var('topic') ):
				...
				require_once('wps_forum_post.php');	

			else:

				require_once('wps_forum_posts.php');
...
function wps_forums($atts) {
	...
	if ($show_header):
		$html .= '<div class="wps_forum_categories_header">';
			$html .= '<div class="wps_forum_categories_description">'.__('Forum', WPS2_TEXT_DOMAIN).'</div>';
			$html .= '<div class="wps_forum_categories_count">'.__('Count', WPS2_TEXT_DOMAIN).'</div>';
			$html .= '<div class="wps_forum_categories_last_poster">'.__('Last Poster', WPS2_TEXT_DOMAIN).'</div>';
			$html .= '<div class="wps_forum_categories_freshness">'.__('Freshness', WPS2_TEXT_DOMAIN).'</div>';
		$html .= '</div>';
	endif;

	$html = wps_forum_categories_children($html, $parent, $show_children, 0);
...
function wps_forum_categories_children($html, $forum_id, $show_children, $indent) {
	...
	if ( count($terms) > 0 ):
		...
		if ($forums):
			...
			foreach ($forums as $forum):
				...
				if ($show_children)
					$html = wps_forum_categories_children ($html, $forum['term_id'], $show_children, $indent+1);
