...
function wps_forum_init() {
	...
	wp_localize_script( 'wps-forum-js', 'wps_forum_ajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );		
...
function wps_forum_show_posts($atts) {
	...
	if ($include_comments):
		...
		global $wpdb;
		$sql = "SELECT * FROM ".$wpdb->prefix."comments c LEFT JOIN ".$wpdb->prefix."posts p ON c.comment_post_ID = p.ID WHERE comment_approved=1 AND p.post_type = %s ORDER BY comment_ID DESC LIMIT %d, %d";
									...
									$forum_html .= '<div class="wps_forum_get_post">';
										...
										if ($show_date):
											$forum_html .= '<div class="wps_forum_get_date">';
												$forum_html .= sprintf($date_format, human_time_diff(strtotime($forum_post[$base_date]), current_time('timestamp', 1)), WPS2_TEXT_DOMAIN);
											$forum_html .= '</div>';										
										endif;
										...
										if ($show_snippet):
											$forum_html .= '<div class="wps_forum_get_snippet">';
												$content = convert_smilies(make_clickable(wpautop(strip_tags($forum_post['post_content']))));
...
function wps_forum_comment($atts) {
	...
	if ((!isset($_GET['forum_action']) || ($_GET['forum_action'] != 'edit' && $_GET['forum_action'] != 'delete')) && get_query_var('topic') ): // showing a single post
		...
		extract( shortcode_atts( array(
            ...
            'no_permission_msg' => __('You do not have permission to comment on this forum.', WPS2_TEXT_DOMAIN),
			'moderate' => false,
			'show' => 1,
		...
		if ($forums):
			...
			foreach ($forums as $forum):
				...
				$forum_html .= '<div class="wps_forum_categories_item">';
                    ...
                    $forum_suffix = apply_filters( 'wps_forum_name_filter', '', $forum['term_id'] );
					$forum_html .= '<h'.($indent+2).'><a href="'.$url.'">'.$forum_name.'</a>'.$forum_suffix.'</h'.($indent+2).'>';
					$forum_html .= '</div>';
				...
				if ($show_posts):
					...
					$forum_html .= wps_forum(array('slug' => $forum['slug'], 'base_date' => $base_date, 'status' => $show_closed, 'show_header' => $show_posts_header, 'count' => $show_posts, 'header_title'=>$header_title, 'header_count'=>$header_count, 'header_last_activity' => $header_last_activity));
				endif;
