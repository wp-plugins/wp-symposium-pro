...
function wps_forum_backto($atts) {
	...
	if ( ( (!isset($_GET['forum_action']) && !isset($_POST['action'])) || (isset($_POST['action']) && $_POST['action'] == 'wps_forum_post_edit') ) && get_query_var('topic') ): // showing a single post
			...
			foreach ($forums as $forum):
				...
				if ($loop->have_posts()):
					while ( $loop->have_posts() ) : $loop->the_post();
						...
						$created = sprintf(__('%s ago', WPS2_TEXT_DOMAIN), human_time_diff(strtotime($post->post_date), current_time('timestamp', 1)), WPS2_TEXT_DOMAIN);
					endwhile;
				else:
					$author = '-';
					$created = '-';
				...
				if ($show_posts):
					...
					$forum_html .= wps_forum(array('slug'=> $forum['slug'], 'status' => $show_closed, 'show_header' => $show_posts_header, 'count' => $show_posts, 'header_title'=>$header_title, 'header_count'=>$header_count, 'header_last_activity' => $header_last_activity));
				endif;
