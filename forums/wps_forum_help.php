	...
	if (isset($the_post['wpspro_quick_start']) && $the_post['wpspro_quick_start'] == 'forum'):
		...
		if (is_wp_error($new_term)):
			...
			echo '<div class="wps_success">';
				...
				echo sprintf(__('If you are using the <a href="%s">Forum Roles Security</a> plugin, choose who can see the forum via <a href="%s">WPS Pro->Forum Setup</a>.', WPS2_TEXT_DOMAIN), "http://www.wpsymposiumpro.com/plugin/forum-roles-security/", "edit-tags.php?taxonomy=wps_forum&post_type=wps_forum_post").'<br />';
...
function wps_admin_getting_started_forum() {
  	...
  	echo '<div class="wps_admin_getting_started_content" id="wps_admin_getting_started_forum" style="display:none">';
  		...
  		echo '<div style="border:1px dashed #333; background-color:#efefef; margin-bottom:10px; padding-left:15px">';
		  	...
		  	echo '<ol>';
			echo '<li>'.sprintf(__('<a href="%s">Create a new WordPress page</a> for your forum (for example "General Forum").', WPS2_TEXT_DOMAIN), 'post-new.php?post_type=page').'</li>';
			echo '<li>'.sprintf(__('Create a forum to match your new page in the <a href="%s">Forum Setup</a> (for example "General Forum").', WPS2_TEXT_DOMAIN), 'edit-tags.php?taxonomy=wps_forum&post_type=wps_forum_post').'</li>';
