...
function wps_post_delete($post_id, $atts) {
	...
	if ($the_post && ($user_can_delete_forum || current_user_can('manage_options'))):
		...
		$html .= wps_bbcode_replace(convert_smilies(make_clickable(wpautop(html_entity_decode($the_post->post_content)))));
...
function wps_comment_delete($the_post, $atts) {
	...
	if ($user_can_delete_comment || current_user_can('manage_options')):
		...
		$html .= wps_bbcode_replace(convert_smilies(make_clickable(wpautop(html_entity_decode($current_comment->comment_content)))));
