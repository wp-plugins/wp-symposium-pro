...
function wps_comment_delete($the_post, $atts) {
	...
	if ($user_can_delete_comment || current_user_can('manage_options')):
		...
		$comment_content = wps_bbcode_replace(convert_smilies(make_clickable(wpautop(esc_html($current_comment->comment_content)))));
...
function wps_save_post($post_data, $files_data, $moved_to) {
	...
	if ($post_id):
		...
		$can = user_can_edit_forum_filter_roles($user_can_edit_forum, $current_post, $current_user->ID, $post_term_term_id);
