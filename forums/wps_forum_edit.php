...
function wps_post_delete($post_id, $atts) {
	...
	if ($the_post && ($user_can_delete_forum || current_user_can('manage_options'))):
        ...
        $the_content = str_replace( array('[', ']'), array('&#91;', '&#93;'), $the_post->post_content );	    
		$html .= wps_bbcode_replace(convert_smilies(wps_make_clickable(wpautop($the_content))));
...
function wps_comment_delete($the_post, $atts) {
	...
	if ($user_can_delete_comment || current_user_can('manage_options')):
        ...
        $the_content = str_replace( array('[', ']',), array('&#91;', '&#93;', '&lt;', '&gt;'), $current_comment->comment_content );	    
		$html .= wps_bbcode_replace(convert_smilies(wps_make_clickable(wpautop($the_content))));
...
function wps_save_post($post_data, $files_data, $moved_to) {
	...
	if ($post_id):
		...
		if ( $user_can_edit_forum || current_user_can('manage_options') ):
            ...
            $title = str_replace( array('[', ']'), array('&#91;', '&#93;'), $post_data['wps_forum_post_edit_title'] );	
            $content = str_replace( array('[', ']'), array('&#91;', '&#93;'), $post_data['wps_forum_post_edit_textarea'] );	
