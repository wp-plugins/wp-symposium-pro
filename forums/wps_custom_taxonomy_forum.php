...
function wps_taxonomy_metadata_edit( $tag ) {
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			<label for="wps_forum_cat_page"><?php _e('ShareThis', WPS2_TEXT_DOMAIN); ?></label>
		</th>
		<td>
			<p><span class="description">
                <?php echo sprintf(__('On <a href="%s" target="_blank">ShareThis</a>, choose "website code" (not Wordpress) and copy the two sets of code supplied below (applies to all forums). Use [wps-forum-sharethis slug="xxx"] to display.', WPS2_TEXT_DOMAIN), "http://www.sharethis.com"); ?>
            </span></p>
            <?php
            $buttons = get_option('wps_forum_sharethis_buttons') ? get_option('wps_forum_sharethis_buttons') : '';
            $js = get_option('wps_forum_sharethis_js') ? get_option('wps_forum_sharethis_js') : '';
            ?>
            <p><strong><?php _e('1. Span tags (code for displaying your buttons)', WPS2_TEXT_DOMAIN); ?></strong></p>
            <textarea style="height:100px;" name="wps_forum_sharethis_buttons"><?php echo $buttons; ?></textarea>
            <p><strong><?php _e('2. Script tags', WPS2_TEXT_DOMAIN); ?></strong></p>
            <textarea style="height:100px;" name="wps_forum_sharethis_js"><?php echo $js; ?></textarea>
		</td>
	</tr> 
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			<label for="wps_forum_featured_image"><?php _e('Featured Image', WPS2_TEXT_DOMAIN); ?></label>
		</th>
		<td>
            <input id="wps_forum_featured_upload_image" type="text" style="width: 50%" name="wps_forum_featured_image" value="<?php echo wps_get_term_meta($tag->term_id, 'wps_forum_featured_image', true); ?>" /> 
            <input id="wps_forum_featured_upload_image_button" type="button" class="button-secondary" value="<?php _e('Select from Media Library', WPS2_TEXT_DOMAIN); ?>" /><br />
            <span class="description"><?php _e('URL of an image that, if set, is used with the [wps-forums] shortcode.', WPS2_TEXT_DOMAIN); ?>
		</td>
	</tr> 
...
function wps_save_taxonomy_metadata( $term_id ) {
	...
	if (isset($_POST['wps_forum_featured_image']))
		wps_update_term_meta( $term_id, 'wps_forum_featured_image', $_POST['wps_forum_featured_image'] );
    ...
    $buttons = isset($_POST['wps_forum_sharethis_buttons']) ? stripslashes($_POST['wps_forum_sharethis_buttons']) : false;
    $js = isset($_POST['wps_forum_sharethis_js']) ? stripslashes($_POST['wps_forum_sharethis_js']) : false;
    update_option( 'wps_forum_sharethis_buttons', $buttons );
    update_option( 'wps_forum_sharethis_js', $js );
