		...
		if ($current_user->ID == $user_id):
			...
			if (isset($_POST['wps_usermeta_change_update'])):
				...
				if (isset($_POST['wpspro_password']) && $_POST['wpspro_password'] != ''):
					...
					wp_set_password($pw, $current_user->ID);
			...
			if (!isset($_POST['wpspro_password']) || $_POST['wpspro_password'] == ''):
				...
				$form_html .= '<form id="wps_usermeta_change" autocomplete="on" action="#" method="POST">';

					$form_html .= '<input type="hidden" name="wps_usermeta_change_update" value="yes" />';

					$value = isset($_POST['wpspro_display_name']) ? stripslashes($_POST['wpspro_display_name']) : $current_user->display_name;
					...
					$value = isset($_POST['wpspro_email']) ? $_POST['wpspro_email'] : $current_user->user_email;
						$form_html .= '<div class="wps_usermeta_change_item">';
					...
					$form_html .= '<input type="submit" id="wps_usermeta_change_submit" class="'.$class.'" value="'.$label.'" />';
...
function wps_usermeta_change_link($atts) {
	...
	if (is_user_logged_in()) {
		...
		if ($current_user->ID == $user_id)
			$html .= '<a href="'.get_page_link(get_option('wpspro_edit_profile_page')).'?user_id='.$user_id.'">'.$text.'</a>';
