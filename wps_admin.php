            ...
            <table><tr>
				<td valign="top" class="wps_release_notes" style="<?php echo $cup_of_tea_left; ?>width:45%;">
					...
					<a href="http://www.wordpress.org/plugins/wp-symposium-pro" target-"_blank">Available from the WordPress repository</a><br />
         			...
         			<h3>Avatar links</h3>
         			<p>If applicable, avatars now link to profile pages.</p>

         			<h3>Edit Profile</h3>
         			<p>Added required_msg to [wps-usermeta-change] (message for mandatory fields)</p>

         			<h3>Profile Page</h3>
         			<p>Added two options to WPS Pro->Setup->Profile Page.
         				<br /><br />The first allows you to choose if alerts are sent out if users are posting activity to all friends, as can be over-whelming if users tend to be friends with a lot of other users on your site.
         				<br /><br />The second is whether to set the page focus on the new activity post area, which may be annoying depending on your page layout.</p>
                         
                    <h3>HTTPS detection</h3>
                    <p>New option under Setup->System Options ("HTTPS detection") which forces true to be returned when checking for presence of HTTPS server (known to overcome some situations when detection fails).</p>    
				</td>
				<td style="width:1%">&nbsp;</td>
				<td valign="top" class="wps_release_notes" style="<?php echo $cup_of_tea_right; ?>">
					...
					<h3>Licence Code</h3>
					<p>Following its introduction in the last release, if you don't enter your licence code, a nag message will be shown on your site (not shown on localhost).
					...
					<h3>New Reward</h3>
					<p>You can now give reward points for when a user logs in or registers on your site. They appear in the "Type" drop-down list, in the Reward Details box.</p>

					<h3>Mandatory Profile Extensions</h3>
					<p>When adding or editing a profile extension, most can be set as mandatory when they are displaying on the Edit Profile Page with the [wps-usermeta-change] shortcode. Users cannot save their edit profile page until the mandatory fields are completed.</p>

					<h3>Forum Signatures</h3>
					<p>These are now shown below any attachments using the wps_forum_item_filter and wps_forum_item_comment_filter filters.</p>
