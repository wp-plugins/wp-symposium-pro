...
function wps_alerts_activity($atts) {
	...
	if ( is_user_logged_in() ):
		...
		foreach ($alerts as $alert):
			...
			$item['post_date_gmt'] = $alert->post_date_gmt;
		...
		if ($list):
			$html .= '<div style="max-width:100%">';
				...
				foreach ($list as $alert):
					...
					$html .= $alert['excerpt'];
					$html .= ' '.sprintf($date_format, human_time_diff(strtotime($alert['post_date_gmt']), current_time('timestamp', 1)), WPS2_TEXT_DOMAIN);
					$html .= '</option>';
				endforeach;
				$html .= "</select>";
			$html .= '</div>';
