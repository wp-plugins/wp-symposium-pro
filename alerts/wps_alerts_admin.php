...
function wps_admin_getting_started_core_hook_alerts($the_post) {
	...
	<tr class="form-field">
		...
		<td>
			<input type="checkbox" style="width:10px" name="wps_force_https" <?php if (get_option('wps_force_https')) echo 'CHECKED '; ?>/><span class="description"><?php _e('Force HTTPS detection to true.', WPS2_TEXT_DOMAIN); ?></span>
		</td>
	</tr> 
	<?php
}
...
function wps_admin_getting_started_core_save_hook_alerts($the_post) {
	...
	if (isset($the_post['wps_force_https'])):
		update_option('wps_force_https', true);
	else:
