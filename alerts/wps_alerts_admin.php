...
function wps_pro_insert_alert($type, $subject, $content, $author_id, $recipient_id, $parameters) {
...
function wps_admin_getting_started_alerts() {
	...
	// Show menu item	
  	...
  	echo '<div class="wps_admin_getting_started_content" id="wps_admin_getting_started_alerts" style="display:none">';
	...
	<?php echo sprintf(__('There are several WordPress plugins available to support this, such as <a href="%s">WP Mail SMTP</a>.', WPS2_TEXT_DOMAIN), "https://wordpress.org/plugins/wp-mail-smtp/"); ?>
		...
		<td>
			<input type="checkbox" name="wps_alerts_check" value="" />
			<span class="description">
				<?php echo sprintf(__('Tick to send a test email to %s.', WPS2_TEXT_DOMAIN), get_bloginfo('admin_email')); ?>
			</span></td> 
		</tr> 	
...
function wps_alerts_admin_options_save ($the_post) {
	...
	if (isset($the_post['wps_alerts_check'])):
		...
		if (wp_mail (get_bloginfo('admin_email'), __('Test email', WPS2_TEXT_DOMAIN), __('Wahoo! It worked!', WPS2_TEXT_DOMAIN))):
...
function wps_alerts_scheduled_job() {
	...
	// Inform admin
	$admin_content = sprintf(__('Started scheduled alerts at %s.', WPS2_TEXT_DOMAIN), current_time('mysql')).'<br /><br />';
	...
	foreach ( $pending_posts as $pending ): 
		if ($c < $max):
			$user = get_user_by('login', get_post_meta( $pending->ID, 'wps_alert_recipient', true ));
			if ($user):
				$recipient = get_post_meta( $pending->ID, 'wps_alert_recipient', true );
				$content = wps_bbcode_replace(convert_smilies(make_clickable(wpautop($pending->post_content))));
				...
				if (wp_mail ($user->user_email, $pending->post_title, $filtered_content, $headers)):
				    ...
				    $c++;
				    // Update post
				    update_post_meta( $pending->ID, 'wps_alert_sent_datetime', current_time('mysql') );
				else:
				    update_post_meta( $pending->ID, 'wps_alert_failed_datetime', current_time('mysql') );
					...
					if (isset($phpmailer)) {
						$msg .= '<br /><em>'.$phpmailer->ErrorInfo.'</em>';
				...
				// Set post to published. If it failed, set status of alert to pending to re-try.
				$this_post = array(
					'ID'           	=> $pending->ID,
					'post_status' 	=> 'publish'
				);
...
function wps_admin_getting_started_core_hook_alerts($the_post) {
	...
	<tr class="form-field">
		<th scope="row" valign="top">
			<label for="wps_core_options"><?php _e('Alerts cleanup', WPS2_TEXT_DOMAIN); ?></label>
		</th>
		<td>
			<input type="checkbox" style="width:10px" name="wps_cleanup" /><span class="description"><?php _e('Run this if you delete users.', WPS2_TEXT_DOMAIN); ?></span>
		</td>
	</tr> 
	</table>
	<?php
